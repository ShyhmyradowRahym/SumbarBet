...
const Gulaklyklar = () => {
    ...
    useEffect(() => {
        async function getData() {
            try {
                const response = await axios.get('/main-page/categories?category=3&subCategory=18');
    ...
    return (
        <div className='w-full'>
            <div className='w-full flex justify-center items-center my-2'>
                <Link to='/category?cat=3&subCat=18&title=gulaklyklar' className='flex items-center hover:text-red-600'>
                    <RiArrowDropRightFill className='text-4xl' />
                    <h1 className='md:text-xl md:font-bold'>GULAKLYKLAR</h1>
                </Link>
            </div>
