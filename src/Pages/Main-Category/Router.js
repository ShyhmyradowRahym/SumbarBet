...
const Router = () => {
    ...
    useEffect(() => {
        async function getData() {
            try {
                const response = await axios.get('/main-page/categories?category=9&subCategory=58');
    ...
    return (
        <div className='w-full'>
            <div className='w-full flex justify-center items-center my-2'>
                <Link to='/category?cat=9&subCat=58&title=routers' className='flex items-center hover:text-red-600'>
                    <RiArrowDropRightFill className='text-4xl' />
                    <h1 className='md:text-xl md:font-bold'> ROUTERLER</h1>
                </Link>
            </div>
