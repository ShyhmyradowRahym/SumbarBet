...
const Plata = () => {
    ...
    useEffect(() => {
        async function getData() {
            try {
                const response = await axios.get('/main-page/categories?category=2&subCategory=2');
    ...
    return (
        <div className='w-full'>
            <div className='w-full flex justify-center items-center my-2'>
                <Link to='/category?cat=2&subCat=2&title=esasy platalar' className='flex items-center hover:text-red-600'>
                    <RiArrowDropRightFill className='text-4xl' />
                    <h1 className='md:text-xl md:font-bold'> ESASY PLATALAR</h1>
                </Link>
            </div>
